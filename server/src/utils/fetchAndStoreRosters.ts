    ...
    try {
        ...
        for (const key in teams) {
            if (Object.prototype.hasOwnProperty.call(teams, key)) {
                ...
                console.log(`Processing ${players.length} players for ${team.triCode}...`);

                const formattedPlayers = players.map((player: { playerId: number; headshot?: string; firstName: { default: string }; lastName: { default: string }; position: { code: string }; jerseyNumber?: number; stats?: { points?: number; goals?: number; assists?: number; plusMinus?: number; saves?: number; goalsAllowed?: number; savePercentage?: number; goalsAgainstAverage?: number } }) => ({
                    id: player.playerId,
                    ...
                    plusMinus: player.position.code === 'G' ? null : player.stats?.plusMinus || 0, // NULL for goalies
                    saves: player.position.code === 'G' ? player.stats?.saves || 0 : null, // NULL for skaters
                    goalsAllowed: player.position.code === 'G' ? player.stats?.goalsAllowed || 0 : null,
                    savePercentage: player.position.code === 'G' ? player.stats?.savePercentage || 0 : null,
                    goalsAgainstAverage: player.position.code === 'G' ? player.stats?.goalsAgainstAverage || 0 : null,
                }));

                console.log(`Upserting ${formattedPlayers.length} players for ${team.triCode}...`);
                await Player.bulkCreate(formattedPlayers, { updateOnDuplicate: ['points', 'goals', 'assists', 'plusMinus', 'saves', 'goalsAllowed', 'savePercentage', 'goalsAgainstAverage'] });
